...
const ghsToBinary = (ghs: string) =>
  ghs
    .split("")
    .map((char) => base32.indexOf(char).toString(2).padStart(5, "0"))
    .join(""); //For some reason I like to use arrow syntax with one-liners, don't know why
...
export function encode([lon, lat]: number[], precision: number): string {
  ...
  const longitudeBinary = longitudeCode
    .toString(2)
    .padStart(longitudeLength, "0");
  const binaryGeoHash =
    latitudeLength == longitudeLength
      ? interleave(longitudeBinary, latitudeBinary)
      : interleave(latitudeBinary, longitudeBinary);
  let geohash = "";
  for (let i = precision - 1; i >= 0; i--) {
    geohash +=
      base32[
        Number((BigInt(parseInt(binaryGeoHash, 2)) >> BigInt(5 * i)) & 31n)
      ];
  }
  return geohash;
}
export function decode(hash: string) {
  ...
  for (let i = 0; i < binaryRep.length; i++) {
    ...
    ranges[coordType][changeIndex] =
      (ranges[coordType][0] + ranges[coordType][1]) / 2;
  }
  return [
